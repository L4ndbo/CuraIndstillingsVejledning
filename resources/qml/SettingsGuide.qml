...
Window {
	...
	title: catalog.i18nc("@title", "Cura Indstillings Vejledning") + " (" + manager.pluginVersion + ")"

	modality: UM.Preferences.getValue("cura_indstillings_vejledning/window+always+in+front") ? Qt.ApplicationModal : Qt.NonModal
	Connections {
		target: UM.Preferences
		onPreferenceChanged: {
			//Update modality if preference changes.
			if(preference !== "cura_indstillings_vejledning/window+always+in+front") {
				return;
			}
			settingsGuideBase.hide(); //Qt limitation: Need to hide and show the window to change the modality.
			settingsGuideBase.modality = UM.Preferences.getValue("cura_indstillings_vejledning/window+always+in+front") ? Qt.ApplicationModal : Qt.NonModal;
